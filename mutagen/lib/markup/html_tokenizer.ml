...
let sequence_to_lowercase s =
  let to_lowercase (l, c) =
    l, to_lowercase c
  in
  s |> List.map to_lowercase
      ...
      | (n, v)::more ->
        if list_mem_string n seen then
          report l (`Bad_token (n, "tag", "duplicate attribute")) !throw
            (fun () ->
          rev_deduplicate ((n, v)::accumulator) (n::seen) more k)
      ...
      | _ ->
        ...
        let finish best matched replace =
          ...
          | Some (text, code_points) ->
            next_option input !throw (function
              | Some (_, 0x003B) -> k (Some (text, code_points))
              | maybe_v ->
                let unterminated () =
          ...
          | _ ->
            peek_n 7 input !throw (function
              | [_, 0x5B; _, 0x43; _, 0x44; _, 0x41;
                ...
                if !foreign () then
                  next_n 7 input !throw (fun _ ->
                  emit (l', `Begin_CDATA) cdata_section_state)
                else
                  report l'
